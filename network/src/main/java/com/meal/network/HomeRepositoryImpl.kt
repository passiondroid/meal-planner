...
class HomeRepositoryImpl(private val mealPlannerApi: MealPlannerApi, private val generativeModel: GenerativeModel) : HomeRepository {
...
//            for (i in 0 until response.categories.size step 3) {
//                val sublist = response.categories.subList(i, i + 3).takeIf { it.isNotEmpty() }
                ...
                try {
                   ...
                   // FileLogger.log("prompt ${GenAIUtil.getPrompt(Locale.getDefault().language, textList)}")
                    ...
                    response.categories.forEachIndexed { index, it ->
                        ...
                        response.categories[index].name = translatedRespone[translatedIndex].translatedText ?: response.categories[index].name
                        translatedIndex += 1
                        response.categories[index].subCategories.forEachIndexed { subcategoryIndex, subcategory ->
                            response.categories[index].subCategories[subcategoryIndex].name = translatedRespone[translatedIndex].translatedText ?: response.categories[index].subCategories[subcategoryIndex].name
                            FileLogger.log("translatedIndex $translatedIndex")
                            FileLogger.log("translatedRespone ${translatedRespone[translatedIndex]}")
                            translatedIndex += 1
                        }
                    ...
                    FileLogger.log(ex.stackTraceToString())
                }
        }
        return response
    }

    private fun getHomePageResponse(): HomePageResponse {
        return HomePageResponse(categories = mutableListOf(
            HomePageResponse.Category(1, "Popular Categories", mutableListOf(
            ...
            HomePageResponse.Category(2, "Cooking Method", mutableListOf(
            ...
            HomePageResponse.Category(3, "Cuisine", mutableListOf(
            ...
            HomePageResponse.Category(4, "Difficulty", mutableListOf(
            ...
            HomePageResponse.Category(5, "Main Protein", mutableListOf(
            ...
            HomePageResponse.Category(6, "Meal Type", mutableListOf(
